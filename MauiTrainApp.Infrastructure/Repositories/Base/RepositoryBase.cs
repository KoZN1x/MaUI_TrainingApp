...
{
    internal abstract class RepositoryBase<TEntity, TRecord> : IRepository<TEntity>
        ...
        where TRecord : BaseRecord
    {
        ...
        protected async Task<ICollection<TEntity>> WhereAsync(
            Expression<Func<TRecord, bool>> predicate,
            CancellationToken cancellationToken = default)
        {
            var records = await Query
        ...
        /// <summary>
        /// Приводит состояние дочерних записей к переданному графу: новые - Added, существующие - Modified,
        /// пропавшие из графа - Deleted. Без этого Update отсоединённого агрегата не добавляет и не удаляет детей.
        /// </summary>
        ...
        private ICollection<TEntity> ToDomain(ICollection<TRecord> records)
        {
            return records
